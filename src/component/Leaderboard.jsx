...
const players = [
  ...
  <button className="flex items-center gap-1 px-4 py-2 text-blue-600 border border-blue-600 rounded-lg hover:bg-blue-50">
    <img src={Icon} className="w-5 h-4 text-blue-600" alt="Compare Icon" />
    Compare
  </button>
    ...
    Share
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 transition-opacity">
          <div
            className={`fixed bottom-0 left-0 w-full bg-white rounded-t-3xl  transition-all duration-300 ease-out ${isOpen ? "translate-y-0" : "translate-y-full"
              }`}
          >
            {/* Drag Handle */}
            ...
            <div className="flex flex-col gap-3">
              ...
              <input
                ...
                className=" px-3 ml-3 mr-3 py-2 border rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500"
              />

              <label className="text-sm pl-4 font-semibold text-gray-600">Note</label>
              <div className=" ml-3 mr-3 px-3 py-2   bg-gray-100 text-gray-500 rounded-full text-sm">
                Info entered here will appear publicly.
              </div>

              <button className="ml-3 mr-3 mb-4 mt-2 bg-blue-600 text-white py-2.5 rounded-lg hover:bg-blue-700 transition-colors">
                Add
              </button>
            </div>
          </div>
      ...
      {isSortOpen && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 transition-opacity">
          <div className={`fixed bottom-0 left-0 w-full bg-white shadow-lg rounded-t-xl  transition-all duration-300 ease-out ${isSortOpen ? "translate-y-0" : "translate-y-full"}`}>
            {/* Close Handle */}
            <div className="flex justify-center ">
            <button
            ...
            {/* Modal Content */}
            <h2 className="text-center text-lg font-bold ">Change Username</h2>
